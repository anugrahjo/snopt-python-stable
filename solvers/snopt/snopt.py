...
def snoptc(userfun,nnObj,nnCon,nnJac,x0,J,**kwargs):
    ...
    name    = kwargs.get('name','')
    usropts = kwargs.get('options',SNOPT_options())
    append   = kwargs.get('append2file', False)
    ...
    if name == '':
        prtfile = usropts.getOption('Print filename')
        summfile = usropts.getOption('Summary filename')
    else:
        if usropts.getOption('Print filename') == 'SNOPT_print.out':
            prtfile = name.strip() + '_SNOPT_print.out'
        else:
            prtfile = usropts.getOption('Print filename')

        if usropts.getOption('Summary filename') == 'SNOPT_summary.out':
            summfile = name.strip() + '_SNOPT_summary.out'
        else:
            summfile = usropts.getOption('Summary filename')

    prtlen  = len(prtfile)
    summlen  = len(summfile)
    # summOn  = 1 if (usropts.getOption('Summary')).lower() == "yes" else 0
    # fsnopt.sninit_wrap(prtfile,prtlen,summOn,snwork.cw,snwork.iw,snwork.rw)
    if append:
        print('Appending to previous SNOPT output files ............')
        fsnopt.sninita_wrap(prtfile,prtlen,summfile,summlen,snwork.cw,snwork.iw,snwork.rw)
    else:
        print('Replacing previous SNOPT output files ............')
        fsnopt.sninitr_wrap(prtfile,prtlen,summfile,summlen,snwork.cw,snwork.iw,snwork.rw)
