...
! Replace previous files with the same name
subroutine sninitr_wrap(prtfile, prtlen, summfile, summlen, cw, lencw, iw, leniw, rw, lenrw)
  implicit none
  character(*),     intent(in)    :: prtfile, summfile
  integer,          intent(in)    :: prtlen, summlen, lencw, leniw, lenrw
  character(8),     intent(inout) :: cw(lencw)
  integer,          intent(inout) :: iw(leniw)
  double precision, intent(inout) :: rw(lenrw)
  !=============================================================================
  ...
  if (prtlen > 0) then
     iPrint = 9
   !   call snFileOpenReplace(iPrint, trim(prtfile), iostat)
   !   call s1trim(trim(prtfile),len)
     open(unit=iPrint, file=trim(prtfile), status='replace', access='sequential', iostat=iostat)
  else
     iPrint = 0
  end if
  ...
  if (summlen > 0) then
     iSumm = 6
   !   call snFileOpenReplace(iSumm, trim(summfile), iostat)
     open(unit=iSumm, file=trim(summfile), status='replace', access='sequential', iostat=iostat)
  else
     iSumm = 0
  end if
...
! Append to previous files with the same name
subroutine sninita_wrap(prtfile, prtlen, summfile, summlen, cw, lencw, iw, leniw, rw, lenrw)
   implicit none
   character(*),     intent(in)    :: prtfile, summfile
   integer,          intent(in)    :: prtlen, summlen, lencw, leniw, lenrw
   character(8),     intent(inout) :: cw(lencw)
   ...
   if (summlen > 0) then
      iSumm = 6
      call snFileOpenAppend(iSumm, trim(summfile), iostat)
   else
      iSumm = 0
   end if
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
! To flush the files to the buffer from Python
